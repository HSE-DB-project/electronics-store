...
from "order" o
         ...
         join employee e on o.employee_id = e.employee_id and o.employee_valid_from = e.valid_from
...
where e.valid_until = '01-01-5999'::date;
...
select sr.name as supplier,
       p.name,
       sxp.quantity,
       s.date
from supply s
         join supplier sr on sr.supplier_id = s.supplier_id
         join supply_x_product sxp on s.supply_id = sxp.supply_id
         join product p on p.product_id = sxp.product_id;
